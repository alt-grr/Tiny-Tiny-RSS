		...
		$ext_tables_part = "";
			...
			if ($view_mode == "adaptive" || $view_query_part == "noscores") {
				if ($search) {
					...
					$unread = $counters->get_feed_unread($feed, $cat_view);
					if ($unread > 0) {
		...
		if ($special) {
			...
			if (db_num_rows($result) > 0) {
				...
				while ($line = db_fetch_assoc($result)) {
					...
					$label_id = -$line['id'] - 11;
					$count = $counters->get_feed_unread($label_id);
...
/*		if (get_pref($link, 'ENABLE_FEED_CATS')) {
		...
		$age_qpart = Counters::get_max_age_subquery();
...
//		$tag = str_replace("+", " ", $tag);
	...
	function api_get_feeds($link, $cat_id, $unread_only, $limit, $offset) {
			...
			if ($cat_id == -4 || $cat_id == -2) {
				$counters = $counters->get_label_counters(true);
			...
			if ($cat_id == -4 || $cat_id == -1) {
				foreach (array(-1, -2, -3, -4, 0) as $i) {
					$unread = $counters->get_feed_unread($i);
			...
			while ($line = db_fetch_assoc($result)) {
				...
				$unread = $counters->get_feed_unread($line["id"]);
...
#					$filename . " (" . $ctype . ")" . "</a>";
	...
	function feedlist_init_feed($link, $feed_id, $title = false, $unread = false, $error = '', $updated = '') {
		...
		if ($unread === false)
			$unread = $counters->get_feed_unread($feed_id, false);
	...
	function handle_public_request($link, $op) {
		...
		case "getUnread":
			...
			if (db_num_rows($result) == 1) {
				...
				print $counters->get_global_unread($uid);

				if ($fresh) {
					print ";";
					print $counters->get_feed_articles(-3, false, true, $uid);
				}
